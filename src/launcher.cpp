/**@file launcher of big memory borrowed and hacked together from code from David Klaftenegger - types changed to unsigned long to accomodate larger values */
#include <iostream>
#include <type_traits>
#include <signal.h>
#include <stdio.h>
...
template<unsigned long SIZE>
	class memory_pool {
		private:
			...
			/** @todo Documentation */
			static thread_local unsigned long max_size;
			/** @todo Documentation */
			static thread_local unsigned long offset;
			static thread_local  unsigned long allocation_counter;
		public:
			 ...
			 */
			static char* reserve( unsigned long size) {
...
template<unsigned long S> thread_local char memory_pool<S>::memory[S];
template<unsigned long S> thread_local unsigned long memory_pool<S>::max_size = S;
template<unsigned long S> thread_local unsigned long memory_pool<S>::offset = 0;
template<unsigned long S> thread_local unsigned long memory_pool<S>::allocation_counter = 0;
...
void* argo_launcher_calloc(size_t nmemb, size_t size) {
	void* nptr = dynamic_alloc(nmemb*size,1);
...
void* argo_launcher_realloc(void* ptr, size_t size) {
	void* nptr = dynamic_alloc(size,1);
...
extern "C" {
	void* dyn_alloc(unsigned long size) {
		return dynamic_alloc(size, 4096);
	}
	...
	void *mremap(void *old_address, size_t old_size,  size_t new_size, int flags, ... /* void *new_address */) {
		if(is_initialized2 && default_global_mempool->is_inside(old_address-4096)) {
